...
class Get_Ava:
    ...
    def replace_ava(self):
        ...
        self.mpada_git = [
            ...
            for root, dirs, files in os.walk("/content/MPADA/", topdown=False)
            for name in files
        ]
        self.origional_py_files = [path for path in self.mpada_git if path[-2:] == "py"]
        self.origional_py_names = [
            path.split("/")[-1] for path in self.mpada_git if path[-2:] == "py"
        ]
    ...
    def cleanup(self):
        ...
        to_clean = [
            root_dir.path
            for root_dir in os.scandir("/content/")
            if root_dir.name not in acceptable_dir
        ]
        clean_command = "rm -rf"
            ...
            if kwargs["download"]:
                for id_ in tqdm(self.files_crypt, colour=("#FF69B4")):
                    if id_ not in (current):
                        gdown.download(id=id_, output="./Images/" + id_, quiet=False)
